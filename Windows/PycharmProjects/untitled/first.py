...
# 将历史成本均价不为0的数据写入到种类表
category_indexby_sku_id = category.set_index('sku_id')
category_mean_cost = pd.concat([category_indexby_sku_id, mean_cost_not_null], axis=1, join='outer')
category_mean_cost.rename(columns={"sku_cost_prc": "mean_cost"}, inplace=True)
